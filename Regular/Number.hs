...
-- Множество представляем характеристической функцией.
type Set a = a -> Bool
...
possible   ::  Language -> Language      -- possible exp  ~  [exp]
possible lang = lang <+> []
...
integral = possible ((symbol '+') `alt` (symbol '-')) `cat` unsigned 

-- Число
number   = integral `cat` possible (symbol '.' `cat` unsigned)
                    `cat` possible (symbol 'e' `cat` integral)
